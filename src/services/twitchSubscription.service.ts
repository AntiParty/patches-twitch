...
// To find this: go to https://www.twitch.tv/antiparty and use browser dev tools
// or use the Twitch API: GET https://api.twitch.tv/helix/users?login=antiparty
const ANTIPARTY_BROADCASTER_ID = process.env.ANTIPARTY_TWITCH_ID || '123456789'; // Replace with actual ID
...
export async function checkAndUpdatePremiumStatus(channelId: number): Promise<SubscriptionStatus> {
    try {
        ...
        try {
            decryptedToken = decryptToken(channel.access_token);
        } catch (decryptError) {
            logger.error(`[TwitchSub] Failed to decrypt token for channel ${channelId}`);
            return {
                isPremium: false,
                tier: null,
                giftedBy: null,
        ...
        const status = await checkTwitchSubscription(decryptedToken, channel.twitch_user_id);
