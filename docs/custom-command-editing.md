...
```
!editcmd rank @{username}, #{rank} {league} - {rankScore} RS | Cashout rank: #{wtRank}
```
...
```
!editcmd peak Peak rank: #{rank} {league} ({rankScore} RS) in {season} | Cashout peak: #{wtRank} in {wtSeason}
```
...
- `{wtRank}` — Peak Cashout rank
- `{wtSeason}` — Peak Cashout season (e.g. "Cashout Season 6")
...
1. Type `!editcmd rank @{username}, #{rank} {league} - {rankScore} RS | Cashout rank: #{wtRank} TEST EDIT` in chat.
2. Type `!editcmd peak Peak rank: #{rank} {league} ({rankScore} RS) in {season} | Cashout peak: #{wtRank} in {wtSeason}` in chat.
